...
def input(filename):
	...
	with open(filename) as f:
		...
		c = [[int(num) for num in line.split()] for line in f] # read the whole distance matrix
...
def SortedCandidates(cur):
	...
	cand.sort(key = lambda x: c[cur][x]) # sort elements in cand in an increasing order of the distance from cur
	return cand
...
def Try(k): # try all values for x[k], being aware x[0],x[1],...,x[k-1], length = f
	global f,x,visited
